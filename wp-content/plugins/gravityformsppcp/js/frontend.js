...
/* harmony import */ var babel_runtime_core_js_object_values__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! babel-runtime/core-js/object/values */ "./node_modules/babel-runtime/core-js/object/values.js");
/* harmony import */ var babel_runtime_core_js_object_values__WEBPACK_IMPORTED_MODULE_3___default = /*#__PURE__*/__webpack_require__.n(babel_runtime_core_js_object_values__WEBPACK_IMPORTED_MODULE_3__);
...
(function ($) {
    ...
    window.GFPPCP = function (args) {
        ...
        self.init = function () {
            ...
            self.paypalButtons = $('#gform_ppcp_smart_payment_buttons');
            self.orderIdField = $('#gf_ppcp_order_id');
            self.totalName = 'gform_ppcp_amount_' + self.formId;

            // Do not support frontend feeds in the AJAX mode.
            if ($('#gform_ajax_frame_' + self.formId).length) {
                self.feedActivated = true;
                self.activeFeed = self.feeds[0];
        ...
        self.bindFrontendFeedsEvaluated = function () {
            gform.addAction('gform_frontend_feeds_evaluated', function (feeds, formId) {
                ...
                for (var i = 0; i < babel_runtime_core_js_object_keys__WEBPACK_IMPORTED_MODULE_4___default()(feeds).length; i++) {
                    if (feeds[i].addonSlug === 'gravityformsppcp' && feeds[i].isActivated) {
                        self.feedActivated = true;

                        for (var j = 0; j < babel_runtime_core_js_object_keys__WEBPACK_IMPORTED_MODULE_4___default()(self.feeds).length; j++) {
                            if (self.feeds[j].feedId === feeds[i].feedId) {
                                self.activeFeed = self.feeds[j];
        ...
        self.bindFormTotalUpdate = function () {
            ...
            gform.addFilter('gform_product_total', function (total, formId) {
                ...
                window[self.totalName] = total;

                // PayPal buttons need to be hidden if the total is 0.
        ...
        self.bindSpinnerTargetElem = function () {
            gform.addFilter('gform_spinner_target_elem', function (elem) {
                return jQuery('#gform_wrapper_' + self.formId + ' .gform_next_button, #gform_send_resume_link_button_' + self.formId + ', #gform_ppcp_smart_payment_buttons');
            });
        };
        ...
        self.getDiscountTotal = function () {
            ...
            if (window['gf_coupons' + self.formId] !== undefined && self.activeFeed.paymentAmount === 'form_total') {
                var size = babel_runtime_core_js_object_values__WEBPACK_IMPORTED_MODULE_3___default()(window['gf_coupons' + self.formId]).length;
                // Add flat discount first.
                for (var i = 0; i < size; i++) {
                    discount = babel_runtime_core_js_object_values__WEBPACK_IMPORTED_MODULE_3___default()(window['gf_coupons' + self.formId])[i];
                    if (discount.type === 'flat') {
                        discountTotal += discount.amount;
                    }
                }
                // Calculate percentage discount AFTER discounted the flat amount.
                for (var _i = 0; _i < size; _i++) {
                    discount = babel_runtime_core_js_object_values__WEBPACK_IMPORTED_MODULE_3___default()(window['gf_coupons' + self.formId])[_i];
                    if (discount.type === 'percentage') {
                        discountTotal += gformRoundPrice((self.itemTotal + self.shipping - discountTotal) * discount.amount / 100);
                    }
                }
            }
        ...
        self.getOrderData = function () {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    data: $('.gfield_price input, .gfield_price select').serialize()
                },
                success: function success(response) {
                    if (response.success) {
                        ...
                        if (self.activeFeed.paymentAmount !== 'form_total') {
                            window[self.totalName] = self.itemTotal;
                        }
        ...
        self.updateOrderDetails = function () {
            var activeFeed = self.activeFeed;
            ...
            var shipping = activeFeed.no_shipping === '1' ? 'NO_SHIPPING' : 'GET_FROM_FILE';
            var shippingDetails = activeFeed.no_shipping === '0' ? {
                name: {
            ...
            var breakdown = {
                item_total: {
                    value: gformRoundPrice(self.itemTotal).toString(),
            ...
            // Get discount.
            var discountTotal = self.getDiscountTotal();
            if (discountTotal > 0) {
                breakdown.discount = {
                    value: discountTotal.toString(),
                    currency_code: self.currency
                };
            }
            ...
            var purchase_units = [{
                amount: {
                    value: gformRoundPrice(window[self.totalName]).toString(),
                    currency_code: self.currency,
                    breakdown: breakdown
                },
        ...
        self.createOrder = babel_runtime_helpers_asyncToGenerator__WEBPACK_IMPORTED_MODULE_2___default()( /*#__PURE__*/babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default.a.mark(function _callee() {
            var orderID;
                ...
                while (1) {
                    switch (_context.prev = _context.next) {
                        case 0:
                            ...
                            return _context.abrupt('return', self.orderIdField.val());

                        case 3:

                            self.countryCode = self.getMergeTagValue(self.activeFeed.address_country);
                            ...
                            _context.next = 7;
                            return self.getCountryCode(self.countryCode);
                        ...
                        case 9:
                            _context.next = 11;
                            return fetch(gforms_ppcp_frontend_strings.ajaxurl + '?action=gfppcp_create_order', {
                                method: 'POST',
                                headers: {
                                    'content-type': 'application/json'
                                },
                                body: babel_runtime_core_js_json_stringify__WEBPACK_IMPORTED_MODULE_1___default()({
                                    nonce: gforms_ppcp_frontend_strings.create_order_nonce,
                                ...
                                if (response.success) {
                                    ...
                                    $(self.form).append($('<input type="hidden" id="gf_ppcp_order_id" name="ppcp_order_id" />').val(orderID));
                                } else {
        ...
        self.renderPayPalButtons = function () {
            // Show the submit button.
            self.toggleSubmitButton('visible');
            // Hide PayPal Smart buttons.
            self.paypalButtons.hide();

            if (gf_check_field_rule(self.formId, self.ccFieldId) !== 'show' || !self.feedActivated || !window[self.totalName] || self.paymentMethod !== 'PayPal Checkout' || !self.isLastPage()) {
                return;
            }

            // Clear the rendered fields.
            $('.ginput_container_custom_card_fields').hide();

            // Hide the submit button.
            self.toggleSubmitButton('hidden');
            // Show the Smart Buttons.
            self.paypalButtons.show();

            if ($('#gform_ppcp_smart_payment_buttons').is(':empty')) {
        ...
        self.renderCustomCardFields = function () {
            if (self.paymentMethod === 'Credit Card' && paypal.HostedFields.isEligible()) {
                if (!$(self.form).data('gfppcpcustomcardrendered')) {
                    var placeholder_1 = $('#input_' + self.formId + '_' + self.ccFieldId + '_1').text();
                    var placeholder_2 = $('#input_' + self.formId + '_' + self.ccFieldId + '_2').text();
                    var placeholder_3 = $('#input_' + self.formId + '_' + self.ccFieldId + '_3').text();
                    ...
                    paypal.HostedFields.render({
                        ...
                        $(self.form).data('gfppcpcustomcardrendered', true);

                        hf.on('cardTypeChange', function (event) {
                            var cardType = void 0;
                            var cardContainer = $('#input_' + self.formId + '_' + self.ccFieldId + '_1').parents('.gfield').find('.gform_card_icon_container');
                            ...
                            if ($('#ppcp_credit_card_type').length) {
                                $('#ppcp_credit_card_type').val(event.cards[0].niceType);
                            } else {
                                $(self.form).append($('<input type="hidden" name="ppcp_credit_card_type" id="ppcp_credit_card_type" />').val(event.cards[0].niceType));
                            }
                        });

                        $(self.form).on('submit', function (event) {
                            if (!self.feedActivated || self.paymentMethod !== 'Credit Card' || $('#gform_save_' + self.formId).val() === '1' || $(this).data('gfppcpsubmitting') || window[self.totalName] <= 0 || !self.isCreditCardOnPage() || self.isGoPrevPage()) {
                                return true;
                            }
                            ...
                            if (!$(self.form).data('gfppcpsubmitting')) {
                                $(self.form).data('gfppcpsubmitting', true);
                            }

                            // Validate CC fields.
                            var state = hf.getState();
                            var formValid = babel_runtime_core_js_object_keys__WEBPACK_IMPORTED_MODULE_4___default()(state.fields).every(function (key) {
                            ...
                            hf.submit(cardData).then(function (payload) {
                                ...
                                if (payload.liabilityShifted === undefined) {
                                    ...
                                    if (payload.authenticationReason === 'SKIPPED_BY_BUYER') {
                                        ...
                                        var result = confirm(gforms_ppcp_frontend_strings.skipped_by_buyer);
                                        if (result) {
                                            $(self.form).submit();
                                        }
                                    } else {
                ...
                // Display Custom Card Fields.
                $('.ginput_container_custom_card_fields').show();
                // Show the submit button.
                self.toggleSubmitButton('visible');
                // Hide PayPal Smart buttons.
                self.paypalButtons.hide();
        ...
        self.displayPaymentMethod = function (paymentMethod) {
            if (!self.feedActivated) {
                $('.gform_ppcp_custom_card_fields').hide();
                self.paymentMethod = '';
            } else {
                // Hide the PayPal field if only PayPal Checkout supported.
                if (babel_runtime_core_js_object_keys__WEBPACK_IMPORTED_MODULE_4___default()(self.paymentMethods).length === 1 && paymentMethod === 'PayPal Checkout') {
                    $('.gform_ppcp_custom_card_fields').hide();
                } else {
                    $('.gform_ppcp_custom_card_fields').show();
                }
                ...
                if (typeof paymentMethod === 'string') {
                    ...
                    // Clear errors in the credit card field.
                    self.displayCardError();
                }
            }

            // Render Custom Card Fields.
            self.renderCustomCardFields();
            // Render PayPal Smart buttons.
            self.renderPayPalButtons();
        };

        self.renderPaymentMethod = function () {
            var paymentMethodField = $('.gform_ppcp_payment_method select');
            var paymentMethod = babel_runtime_core_js_object_keys__WEBPACK_IMPORTED_MODULE_4___default()(self.paymentMethods).length > 1 ? paymentMethodField.val() : self.paymentMethods[0];

            self.displayPaymentMethod(paymentMethod);

            paymentMethodField.on('change', self.displayPaymentMethod);
        };

        self.toggleSubmitButton = function (status) {
            $('#gform_submit_button_' + self.formId).css('visibility', status);
        };
        ...
        self.isCreditCardOnPage = function () {
            ...
            if (!self.ccPage || !currentPage) return true;
        ...
        self.isCardSupported = function (card) {
            if ($.inArray(card.type, babel_runtime_core_js_object_keys__WEBPACK_IMPORTED_MODULE_4___default()(window['gf_cc_rules'])) === -1) {
        ...
        self.displayCardError = function (message) {
            ...
            if (!$(self.GFCCField).find('.validation_message').length) {
                $(self.GFCCField).append('<div class="gfield_description validation_message"></div>');
            }

            var cardErrors = $(self.GFCCField).find('.validation_message');

            if (message !== '') {
                $(self.GFCCField).addClass('gfield_error');
                $(cardErrors[0]).html(message);
                // Hide spinner.
                if ($('#gform_ajax_spinner_' + self.formId).length > 0) {
                    $('#gform_ajax_spinner_' + self.formId).remove();
                }
            } else {
                $(self.GFCCField).removeClass('gfield_error');
                $(cardErrors[0]).html('');
            }
        };
