...
/**
 * The PayPal field is a payment methods field used specifically by the PayPal Commerce Platform Add-On.
 *
 * @since 1.0
 *
...
class GF_Field_PayPal extends GF_Field_CreditCard {
	...
	public function get_field_input( $form, $value = array(), $entry = null ) {
		...
		if ( ! gf_ppcp()->initialize_api() ) {
			if ( ! $is_admin ) {
				return sprintf( esc_html__( '%sPlease check your PayPal Commerce Platform Add-On settings. Your account is not connected yet.%s' ), '<div class="gfield_description validation_message">', '</div>' );
			} else {
		...
		if ( ! $is_admin && ! gf_ppcp()->has_feed( $form['id'] ) ) {
			return sprintf( esc_html__( '%sPlease check if you have activated a PayPal Commerce Platform feed for your form.%s' ), '<div class="gfield_description validation_message">', '</div>' );
		}
		...
		if ( $position === 1350 ) {
			if ( gf_ppcp()->is_custom_card_fields_supported() ) {
                ...
                <span class="section_label">
					...
					<ul>
						<li>
							<input type="checkbox" id="paypal_payment_smart_payment_buttons" value="PayPal Checkout"/>
