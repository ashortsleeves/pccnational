	...
	public function maybe_display_account_status_notices() {
		...
		if ( ! $this->initialize_api() ) {
			if ( ! rgar( $seller, 'primary_email_confirmed' ) ) {
				...
				$message = sprintf(
					/* translators: 1: Open link tag 2: Close link tag */
					esc_html__( 'Your PayPal account currently cannot receivable payments. Please contact PayPal for more details. %1$sLearn more here%2$s.', 'gravityformsppcp' ),
	...
	public function scripts() {
		...
		$scripts = array(
			...
			array(
				...
				'version'   => $this->_version,
				'deps'      => array( 'jquery', 'gform_json', 'gform_gravityforms', 'gform_paypal_sdk' ),
				'in_footer' => false,
				'enqueue'   => array(
				...
				'strings'   => array(
					...
					'card_info_error'     => wp_strip_all_tags( __( 'Your credit card info is invalid. Please check them and try again.', 'gravityformsppcp' ) ),
					'threed_secure_error' => wp_strip_all_tags( __( 'You have not passed the 3D secure authentication. Please try again.', 'gravityformsppcp' ) ),
					'skipped_by_buyer'    => wp_strip_all_tags( __( 'Are you sure to skip the 3D secure authentication?', 'gravityformsppcp' ) ),
...
';
				...
				?>
				<div class="alert_red" style="padding:20px; padding-top:5px;">
					<h4><?php esc_html__( 'SSL Certificate Required', 'gravityformsppcp' ) ?></h4>
					<?php
	...
	public function add_custom_data( $tag, $handle ) {
		if ( $handle === 'gform_paypal_sdk' ) {
			...
			$tag = preg_replace( "/(src=\'.*\')/", "$1 data-client-token='$client_token' data-partner-attribution-id='RocketGenius_PCP'", $tag );
		}
	...
	public function authorize_product( $feed, $submission_data, $form, $entry ) {
		...
		// Turn them both to string or the float number comparison could get you into trouble.
		$payment_amount = GFCommon::remove_currency_symbol( GFCommon::to_money( rgar( $submission_data, 'payment_amount' ), $entry['currency'] ), $entry['currency'] );
		$order_total    = strval( rgars( $order, 'purchase_units/0/amount/value' ) );
		...
		if ( $this->get_intent( $form['id'] ) === 'authorize' ) {
	...
	public function callback() {
		...
		if ( $type === 'PAYMENT.CAPTURE.COMPLETED' ) {
			$links = rgars( $event, 'resource/links' );
			foreach ( $links as $link ) {
				if ( $link['rel'] === 'up' ) {
					$transaction_id = end( explode( '/', $link['href'] ) );
					$entry_id       = $this->get_entry_by_transaction_id( $transaction_id );

					break;
				}
			}
		} elseif ( $type === 'PAYMENT.AUTHORIZATION.VOIDED' ) {
			$transaction_id = rgars( $event, 'resource/id' );
			$entry_id       = $this->get_entry_by_transaction_id( $transaction_id );
		} else {
	...
	public function get_webhook_event() {
		...
		if ( rgar( $result, 'verification_status' ) === 'SUCCESS' ) {
			...
			$error = $this->add_paypal_debug_id( esc_html__( 'Wehook verification failed.', 'gravityformsppcp' ), $result );
	...
	public function get_action_url( $environment = 'live' ) {
		...
		if ( ! $action_url || ( $expires_in + 32300 < time() ) ) { // The partner access token expires in 32400 seconds.
			...
			if ( rgar( $response_body, 'payload' ) ) {
				$payload                         = unserialize( base64_decode( $response_body['payload'] ) );
	...
	public function ajax_create_order() {
		...
		$order = $this->api->create_order( array_merge( array( 'intent' => strtoupper( $this->get_intent( rgpost( 'form_id' ) ) ) ), $payment_details ) );
